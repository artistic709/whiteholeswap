...
contract whiteholeswap is ERC20Mintable {
    using SafeMath for *;
    ...
    IERC20 constant token0 = ERC20(0x6B175474E89094C44Da98b954EedeAC495271d0F); //DAI
    IERC20 constant token1 = ERC20(0x57Ab1ec28D129707052df4dF418D58a2D46d5f51); //SUSD
    ...
    function addLiquidityImbalanced(uint256 token0_in, uint256 token1_in, uint256 share_min) external returns (uint256 share) {
        ...
        share = kAfter.sqrt().mul(_totalSupply).div(kBefore.sqrt()).mul(fee).div(BASE);
